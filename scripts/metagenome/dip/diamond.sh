...
for FILE in `ls -1 *subsample_1*`
do
        SAMPLE=`echo $FILE | cut -d "_" -f1,2`
	echo "Running DIAMOND for $SAMPLE"
        diamond blastx --threads $THREADS --db $DATABASE --query ${SAMPLE}_subsample_1.part_001.fq.gz ${SAMPLE}_subsample_2.part_001.fq.gz --sensitive --log --out ${OUTDIR}/${SAMPLE}.daa --outfmt 100 -k 10 -b7.0 -c1
