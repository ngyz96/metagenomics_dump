...
for FILE in `ls -1 *subsample_2*`
do
        SAMPLE=`echo $FILE | cut -d "_" -f1,2`
	echo "Running DIAMOND for $SAMPLE"
        diamond blastx --threads $THREADS --db $DATABASE --query ${SAMPLE}_subsample_2.fq.gz --sensitive --log --out ${OUTDIR}/${SAMPLE}_2.daa --outfmt 100 -k 10 -b7.0 -c1
